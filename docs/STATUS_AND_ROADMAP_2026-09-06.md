...
5. Validate the preconcentrator experimentally. A mass-balance and timing screen identifies a 500 mL to 5 mL, 80% recovery reference design, and a locked 115-run benchtop protocol now defines recovery, humid-gas breakthrough, blank, carryover and residual-background tests. Physical execution remains open. See `docs/PRECONCENTRATOR_MASS_BALANCE_2026-09-07.md` and `docs/BENCHTOP_VALIDATION_PROTOCOL_2026-09-07.md`.
...
The next step requires laboratory hardware and certified standards: execute the benchtop protocol beginning with system blanks and the 500 mL recovery block. Before hardware is available, the remaining useful software step is an analysis template that converts experimental run results into pass/conditional-pass/fail decisions. Expanding the spectral search remains useful only if a candidate clears both clinical replication and chemical-identity gates.
