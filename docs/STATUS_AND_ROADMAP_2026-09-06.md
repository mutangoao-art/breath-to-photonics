...
The next recommended implementation step is a preconcentrator mass-balance and timing model with explicit sample volume, recovery, trap capacity, desorption volume and cycle time. Expanding the spectral search remains useful only if a candidate clears both clinical replication and chemical-identity gates. External line data are cached with download metadata and excluded from Git.
