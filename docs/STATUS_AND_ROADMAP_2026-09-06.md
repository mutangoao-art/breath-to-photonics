...
5. Validate the preconcentrator experimentally. A mass-balance and timing screen is complete and identifies a 500 mL to 5 mL, 80% recovery reference design. Compound-specific recovery, humid-gas breakthrough, blank, carryover and residual-water measurements remain open. See `docs/PRECONCENTRATOR_MASS_BALANCE_2026-09-07.md`.
...
The next recommended implementation step is a written benchtop validation protocol and acceptance criteria for compound recovery, breakthrough, residual water, blanks and carryover. Expanding the spectral search remains useful only if a candidate clears both clinical replication and chemical-identity gates. External line data are cached with download metadata and excluded from Git.
