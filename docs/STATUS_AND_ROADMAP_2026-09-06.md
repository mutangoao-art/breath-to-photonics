...
4. Refine the instrument noise/drift model and compare direct multipass, cavity-enhanced, and preconcentration-assisted architectures. A first idealized transmission-noise and background-repeatability budget is complete; wavelength drift, 1/f noise, averaging time and sampling losses remain open. See `docs/NOISE_AND_DRIFT_BUDGET_2026-09-07.md`.
...
The next recommended implementation step is to compare optical architectures against the new precision bounds, including explicit path length, enhancement factor, averaging time and preconcentration assumptions. Expanding the spectral search remains useful only if a candidate clears both clinical replication and chemical-identity gates. External line data are cached with download metadata and excluded from Git.
